...
export function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Uğurlu girişdən sonra onAuthStateChange (App.jsx-də) avtomatik
      // profili yükləyib AppShell-ə keçəcək — burada əlavə naviqasiya lazım deyil.
    } catch (err) {
      setError(t("login_error") || "E-poçt və ya parol yanlışdır");
      setLoading(false);
    }
  };
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-8 lg:px-16">
          <div className="w-full max-w-[400px] page-enter">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Field label={t("login_email")} type="email" value={email}
                     onChange={(e) => setEmail(e.target.value)}
                     placeholder="ad.soyad@sirket.az" required />
              ...
              {error && (
                <div className="text-xs px-3 py-2 fade-in"
                     style={{ background: theme.dangerBg, color: theme.danger, borderRadius: 2 }}>
                  {error}
                </div>
              )}
