...
import { flattenStructure } from "../lib";
...
export function Sidebar({ route, setRoute, onClose }) {
  ...
  const allRows = useMemo(() => flattenStructure(structure, lang), [structure, lang]);
  const completedCount = Object.values(analyses).filter(a => a.status === "completed").length;
  const totalPositions = allRows.length;

  return (
    // h-screen + sticky kontekst: bütün sidebar viewport hündürlüyündə sabitlənir,
    // daxili nav scroll olur, brend/profil bölmələri həmişə görünür qalır
    <aside className="w-64 h-screen flex flex-col sticky top-0"
           style={{ background: theme.sidebar, borderRight: `1px solid rgba(255,255,255,0.05)` }}>

      {/* Logo — sabit (scroll olmur) */}
      <div className="flex-shrink-0 px-5 py-5 flex items-center justify-between"
      ...
      {/* Tətbiq adı — sabit */}
      <div className="flex-shrink-0 px-5 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        {/* Tərəqqi göstəricisi — scroll olan hissənin daxilində */}
